...
> 测试方式：3 个独立 taihu bench storage 进程并发，各绑一盘；仅测 4MiB 对象读写
...
|------|------|
| 服务器 | 128.12（96 核） |
| 编译产物 | `taihu bench storage`（efaddee，Go 1.23.12，aarch64，经编译部署 skill：commit→打包 dist/→同步→`go build ./cmd/taihu bench storage`） |

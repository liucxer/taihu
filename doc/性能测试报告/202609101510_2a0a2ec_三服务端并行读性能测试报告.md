...
验证多实例部署的聚合读能力：3 个 taihu server（各绑一块 NVMe）× 3 个 taihu bench cluster 客户端（各 32 线程 / 16 连接 / 4M 对象）并行读时，物理机 CPU、聚合应用层带宽、延迟、loopback 网络、三盘磁盘的真实表现，并采集 3 客户端 + 3 服务端 pprof 定位聚合上限的来源。

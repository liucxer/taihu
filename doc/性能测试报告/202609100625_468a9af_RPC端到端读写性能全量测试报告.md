...
| 编译产物 | `/tmp/taihu server.new`、`/tmp/taihu bench cluster.new`、`/tmp/taihu bench storage.new`（go1.24，GOTOOLCHAIN=auto 补齐 go1.25.0） |
...
| 压测参数 | `taihu bench cluster -addr 100.71.128.12:50051 -mode write/read -size 4194304 -threads 16 -latency [-cpuprofile]` |

## 3. 测试方法

1. **单盘极限标定**：本地 `taihu bench storage`（同机 O_DIRECT，不经 gRPC），16 线程并发，测写/读最大带宽作为盘的真上限；另用 `dd` 直测裸盘作为下限参照。
2. **RPC 单客户端基线**：1 个 `taihu bench cluster`（16 线程）先写后读同一批 4MiB key，各 10000 个（40GB/轮）。

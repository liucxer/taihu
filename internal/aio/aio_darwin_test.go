...
// 兜底实现（ring_fallback_other.go），没有 libaio、没有 io_uring，也没有 O_DIRECT（darwin 的
// x/sys/unix 里根本没有 O_DIRECT 符号），故只跑普通缓冲 IO 这一条通道。
//
// 契约本体（runRingContract 与选项用例）随本文件附带，与 aio_linux_test.go 各持一份。
...
// fallbackBackend macOS 上唯一可跑的后端：ring_fallback_other.go 的 goroutine 兜底。

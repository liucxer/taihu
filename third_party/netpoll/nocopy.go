...
const (
	...
	// flagAligned marks a buffer node whose memory was allocated by the aligned
...
// netpoll node buffers come from the aligned allocator (bufpool), so receive buffers
// are 4K-aligned and pooled/reused instead of being allocated via netpoll's mcache;
// on Release (node fully consumed) the buffer returns to the aligned pool via put.
// This reduces heap allocation/GC without transferring node ownership to callers.
var (
	alignedAllocGet func(n int) []byte
	alignedAllocPut func(b []byte)
)

// SetAlignedAllocator injects a pluggable aligned node allocator. get(n) must return
// a slice with len/cap >= n whose first byte address is aligned (e.g. from a 4K-aligned
// buffer pool); put must return a slice previously produced by get. When unset (or nil)
// netpoll keeps its built-in mcache allocator, matching upstream behaviour. The hook
// only affects nodes created after it is installed, so it is safe to call once at startup.

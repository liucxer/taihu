...
const (
	...
	// alignSize 共享内存 buffer 数据区对齐粒度（O_DIRECT 直读要求 4K）。
	// 布局改造：buffer 起始按 alignSize 步进，且 bufferRegion 起始偏移经
	// alignListOffset 调整为 (4096k-20)，使每个 buffer 的 data 区（起始+20B）
	// 恰好 4K 对齐，可直接作为 O_DIRECT 读缓冲（免 memcpy 直读共享内存）。
	alignSize = 4096
)

// align4K 将 n 向上取整到 alignSize 的倍数。
func align4K(n uint32) uint32 {
...
func createBufferManager(listSizePercent []*SizePercentPair, path string, mem []byte, offset uint32) (*bufferManager, error) {
	...
	hadUsedOffset := uint32(bufferManagerHeaderSize) + offset
	// 列表起始对齐：使每个 buffer 的 data 区（起始+20B）4K 对齐（O_DIRECT 直读）。
	hadUsedOffset = alignListOffset(hadUsedOffset)
	bufferRegionCap := uint64(len(mem)) - uint64(hadUsedOffset) - uint64(bufferListHeaderSize)*uint64(len(listSizePercent))
	...
	*(*uint32)(unsafe.Pointer(&mem[offset+bmCapOffset])) = hadUsedOffset - uint32(bufferManagerHeaderSize) - offset
	return ret, nil
}
...
func mappingBufferManager(path string, mem []byte, bufferRegionStartOffset uint32) (*bufferManager, error) {
	...
	// 与 createBufferManager 相同的列表起始对齐（两端布局一致）。
	hadUsedOffset = alignListOffset(hadUsedOffset)
...
func countBufferListMemSize(bufferNum, capPerBuffer uint32) uint32 {
	// 步长 4K 对齐：保证每个 buffer 的 data 区 4K 对齐（O_DIRECT 直读）。
	return bufferListHeaderSize + bufferNum*align4K(capPerBuffer+bufferHeaderSize)
}
...
func createFreeBufferList(bufferNum, capPerBuffer uint32, mem []byte, offsetInMem uint32) (*bufferList, error) {
	...
	*b.cap = bufferNum
	*b.head = 0
	stride := align4K(capPerBuffer + bufferHeaderSize)
	*b.tail = (bufferNum - 1) * stride
...
func mappingFreeBufferList(mem []byte, offset uint32) (*bufferList, error) {
	...
	b := &bufferList{
		...
		counter:      (*int32)(unsafe.Pointer(&mem[offset+20])),
		offsetInShm:  offset,
	}

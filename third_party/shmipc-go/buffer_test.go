...
func TestLinkedBuffer_ReadString(t *testing.T) {
	initShm()
	factory := func() *linkedBuffer {
		buf, err := bm.allocShmBuffer(1024)
		if err != nil {
	...
	testBufferReadString(t, factory)
}
...
func TestLinkedBuffer_FallbackWhenWrite(t *testing.T) {
	// fork 的 buffer stride 按 4K 对齐，同样内存段内可容纳的 buffer 数量比上游少，
	// 这里放大内存段以保证能分配出 buffer（pop 会保留最后一个 buffer 不出借）。
